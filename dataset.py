...
class Dataset(data.Dataset):
    def __init__(self, root, dataset_name='modelnet40', class_choice=None,
            ...
            segmentation=True, random_rotate=False, random_jitter=False, 
        ...
        if self.class_choice != None:
            indices = [i for i in range(len(self.name)) if self.name[i] == class_choice] # (self.name == class_choice).squeeze()
            ...
            if self.load_file:
                ...
                names = []
                for i in indices:
                    files.append(self.file[i])
                    names.append(self.name[i])
                self.file = files
                self.name = names
...
if __name__ == '__main__':
    ...
    d = Dataset(root=root, dataset_name=dataset_name, class_choice='airplane', 
                        num_points=2048, split=split, random_rotate=False, load_name=True)
    print("datasize:", d.__len__())

    item = 0
    pts, lb, seg, n, f = d[item]

    print(pts.size(), pts.type(), lb.size(), lb.type(), n, f) 